...
def process_sheet(
    ...
    # 3. Reset this sheet's output folder (re-runs never mix stale artifacts with
    #    fresh ones), then STREAM the pipeline: each stage goes to the display
    #    side channel and to disk the moment it is produced (FR-11).
    artifacts.reset_sheet_output(sheet_id)
    stages, run = run_pipeline_with_detection(raw, len(info_file.students))
    for stage in stages:
        if on_stage is not None:
            on_stage(stage)
        artifacts.save_stage(sheet_id, stage)
    sheet_result = run.sheet_result
